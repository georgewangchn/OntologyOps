...
本项目教的是**一个范式怎么推理**（OWL 本体 + SWRL + HermiT）。当你要把这套 OWL 推理**工程化成一个活的、可持续演化的知识资产**——本体像代码一样被版本化、被 PR、被 CI、由 LLM 构建而由推理机裁判——那就是 **[OntologyOps](../../README.md) 的「Ontology Repo」支柱**（正式协议见 [`ontologyops/`](../../ontologyops/)），其可运行的参考实现是 **[GovernanceOps](https://github.com/georgewangchn/GovernanceOps)**（`govops onto …` 一套命令）。

> 📌 **注意分层**：本项目（及 P2–P6、PL1–PL6）是**教学范式演示**，目的是讲清各推理范式，**不以「符合 OntologyOps 协议」为目标**——协议是为**生产级、会持续演化的本体仓库**准备的工程纪律。P1 只是那条弧线的起点：先看懂 OWL 怎么推理，再理解为什么它值得被工程化治理。
